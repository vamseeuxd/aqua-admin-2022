...
export class ProjectListPage {
  ...
  constructor(
    ...
    const projectsQuery = query(projectsRef, where('email', '==', 'vamsi.flex@gmail.com'));
    this.projectList$ = collectionData(projectsQuery, {idField: 'id'});
  }

  ionViewDidEnter() {
  }

  async addNewProject(): Promise<any> {
    const modal = await this.modalController.create({
      component: AddOrEditProjectComponent,
    });
    return await modal.present();
  }

  async editProject(data: any): Promise<any> {
    const modal = await this.modalController.create({
      component: AddOrEditProjectComponent,
      componentProps: {
        isNew: false,
        ...
        buttons: [
          {
            ...
            handler: async () => {
              ...
              try {
                ...
                await loading.dismiss();
                const toast = await this.toastController.create({
                  message: 'Technical Error while Deleting the Project',
                  duration: 2000
                });
                await toast.present();
              }
